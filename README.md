...
- 自定义消息发送：
  - 手机端输入目标眼镜应用包名和任意字符串
  - 通过 `BleMsg(Action.APP_DATA, pkg, Payload(data = "..."))` 发送到眼镜端 Content Center
  - Content Center 按 `pkg` 查找目标应用并转发消息
  - 目标眼镜应用需要接入 [GTBle](https://github.com/AdamTaurus/GTBle) 的 `content_sdk` 才能接收消息
- Wi-Fi 图片查看：
...
手机端只负责把消息发到眼镜端 Content Center，`pkg` 必须填写眼镜端目标应用的真实包名。Content Center 会按这个包名查找目标应用并转发，目标眼镜应用需要接入 [GTBle](https://github.com/AdamTaurus/GTBle) 的 `content_sdk`。
...
ContentCenterManager.addListener(object : ContentCenterManager.Callback {
    override fun onCtrlReceive(message: String) {
        // message 是 Content Center 按包名转发过来的协议 JSON。
        // 手机端发送的字符串在 data.data 字段中。
    }
})
```
...
自定义消息页面演示“手机端 App -> 眼镜端 Content Center -> 目标眼镜 App”的转发链路：

- 手机端输入目标眼镜 App 的包名和要发送的字符串
- 字符串放入 `Payload.data`
- 手机端发送 `Action.APP_DATA` 协议消息到 Content Center
- Content Center 根据 `pkg` 查找并转发给对应眼镜 App
- 目标眼镜 App 必须接入 [GTBle](https://github.com/AdamTaurus/GTBle) 的 `content_sdk`，否则只能到 Content Center，业务 App 收不到
